                    ...
                    @foreach ($transactionProducts as $index => $transactionProduct)
                        <div class="field">
                            <label class="label" for="amount{{ $index }}">
                                {{ $transactionProduct['product']['name'] }} (&euro; {{ $transactionProduct['product']['price'] }}) @lang('admin/inventories.crud.amount')
                            ...
                            <div class="control">
                                <input class="input @error('transactionProducts.{{ $index }}.amount') is-danger @enderror" type="number" min="1"
                                    id="amount{{ $index }}" form="createTransaction" wire:model="transactionProducts.{{ $index }}.amount" required>
                            </div>
